...
def create_accuracy_comparison_chart(overall_results, question_type_results, output_file):
    ...
    plt.title('Overall Accuracy Comparison', fontsize=12)
    ...
    plt.title('Accuracy by Question Type', fontsize=12)
    plt.ylabel('Accuracy', fontsize=10)
    plt.xticks([pos + width/2 for pos in x], question_types, rotation=45, ha='right', fontsize=9)
    plt.ylim(0, 1)
...
def create_type_analysis_chart(type_analysis_results, output_file):
    ...
    plt.title('Accuracy by Knowledge Type', fontsize=12)
    plt.ylabel('Accuracy', fontsize=10)
    plt.xticks([pos + width/2 for pos in x], type_analysis, rotation=45, ha='right', fontsize=9)
    plt.ylim(0, 1)
    plt.legend(fontsize=9)
...
def create_score_comparison_chart(overall_results, question_type_results, output_file):
    ...
    plt.title('Overall Score Comparison', fontsize=12)
    ...
    plt.title('Score by Question Type', fontsize=12)
    plt.ylabel('Average Score', fontsize=10)
    plt.xticks([pos + width/2 for pos in x], question_types, rotation=45, ha='right', fontsize=9)
    plt.ylim(0, 1)
    plt.legend(fontsize=9)
...
def generate_report(overall_results, question_type_results, type_analysis_results):
    ...
    report.append(f"Comparison: {list(overall_results.keys())[0]} vs {list(overall_results.keys())[1]}\n\n")
    ...
    report.append("| Metric | GPT-3.5 Turbo | Claude 3 Haiku | Difference |")
    report.append("|--------|----------------|---------------|-----------|")
    report.append(f"| Total Questions | {gpt_data['total_questions']} | {claude_data['total_questions']} | - |")
    report.append(f"| Correct | {gpt_data['correct']} | {claude_data['correct']} | +{correct_diff} |")
    report.append(f"| Incorrect | {gpt_data['incorrect']} | {claude_data['incorrect']} | -{abs(correct_diff)} |")
    report.append(f"| Accuracy | {gpt_data['accuracy']:.2%} | {claude_data['accuracy']:.2%} | +{accuracy_diff:.2%} |")
    report.append(f"| Average Score | {gpt_data['average_score']:.2f} | {claude_data['average_score']:.2f} | +{score_diff:.2f} |")
    report.append("\n")
    
    # 按题目类型对比
    report.append("## Comparison by Question Type\n")
    if question_type_results:
        ...
        for qt in question_types:
            ...
            report.append(f"### Question Type: {qt}\n")
            report.append("| Metric | GPT-3.5 Turbo | Claude 3 Haiku | Difference |")
            report.append("|--------|----------------|---------------|-----------|")
            report.append(f"| Total Questions | {gpt_qt_data['total_questions']} | {claude_qt_data['total_questions']} | - |")
            report.append(f"| Correct | {gpt_qt_data['correct']} | {claude_qt_data['correct']} | +{qt_correct_diff} |")
            report.append(f"| Accuracy | {gpt_qt_data['accuracy']:.2%} | {claude_qt_data['accuracy']:.2%} | +{qt_accuracy_diff:.2%} |")
            report.append(f"| Average Score | {gpt_qt_data['average_score']:.2f} | {claude_qt_data['average_score']:.2f} | +{qt_score_diff:.2f} |")
            report.append("\n")
    
    # 按知识点类型对比
    report.append("## Comparison by Knowledge Type\n")
    if type_analysis_results:
        ...
        for ta in type_analysis:
            ...
            report.append(f"### Knowledge Type: {ta}\n")
            report.append("| Metric | GPT-3.5 Turbo | Claude 3 Haiku | Difference |")
            report.append("|--------|----------------|---------------|-----------|")
            report.append(f"| Total Questions | {gpt_ta_data['total_questions']} | {claude_ta_data['total_questions']} | - |")
            report.append(f"| Correct | {gpt_ta_data['correct']} | {claude_ta_data['correct']} | +{ta_correct_diff} |")
            report.append(f"| Accuracy | {gpt_ta_data['accuracy']:.2%} | {claude_ta_data['accuracy']:.2%} | +{ta_accuracy_diff:.2%} |")
            report.append(f"| Average Score | {gpt_ta_data['average_score']:.2f} | {claude_ta_data['average_score']:.2f} | +{ta_score_diff:.2f} |")
            report.append("\n")
    
    # 总结
    report.append("## Summary\n")
    report.append(f"1. **Overall Performance**: {claude_name} outperforms {gpt_name} in both accuracy and average score\n")
    report.append(f"2. **Accuracy Improvement**: {claude_name} achieves {accuracy_diff:.2%} higher accuracy than {gpt_name}\n")
    report.append(f"3. **Correct Answers**: {claude_name} answers {correct_diff} more questions correctly than {gpt_name}\n")
    ...
    if best_improvement_ta:
        report.append(f"4. **Most Significant Improvement**: {claude_name} achieves {best_improvement:.2%} higher accuracy in '{best_improvement_ta}' knowledge type\n")
    
    report.append("\n**Conclusion**: Claude 3 Haiku demonstrates superior performance compared to GPT-3.5 Turbo, especially in complex knowledge types like cross-tactic correlation analysis.")
...
def main():
    ...
    if len(model_files) != 2:
        print("Error: Need summary files for two models")
        return
    ...
    print(f"Analysis completed! Results saved to:\n- Detailed analysis: {analysis_file}")
    print(f"- Report: {report_file}")
    print(f"- Accuracy comparison chart: {accuracy_chart}")
    print(f"- Type analysis comparison chart: {type_analysis_chart}")
    print(f"- Score comparison chart: {score_chart}")
